...
/**
 * Builds a ready-to-use Valhalla config JSON string. The template is the standard mobile
 * configuration (loki/thor/odin/meili/service_limits); only the device-specific `mjolnir` paths are
 * substituted, so callers just supply where the data lives.
 *
 * @param tileExtract absolute path to the routing tiles archive (`valhalla_tiles.tar`); Valhalla
 *   mmaps it directly (no extraction).
 * @param admin       absolute path to `admins.sqlite`; defaults to a sibling of [tileExtract]. Missing
 *   is tolerated (Valhalla logs a warning and routes without admin-based country logic).
 * @param timezone    absolute path to `timezones.sqlite`; defaults to a sibling of [tileExtract].
 */
fun defaultValhallaConfig(
    tileExtract: String,
    admin: String? = null,
    timezone: String? = null,
): String {
    val dir = tileExtract.substringBeforeLast('/', ".")
    return CONFIG_TEMPLATE
        .replace(TILE_EXTRACT_TOKEN, tileExtract)
        .replace(ADMIN_TOKEN, admin ?: "$dir/admins.sqlite")
        .replace(TIMEZONE_TOKEN, timezone ?: "$dir/timezones.sqlite")
}

private const val TILE_EXTRACT_TOKEN = "__TILE_EXTRACT__"
private const val ADMIN_TOKEN = "__ADMIN__"
private const val TIMEZONE_TOKEN = "__TIMEZONE__"

// Bundled default config from valhalla-mobile. Only the mjolnir tile_extract/admin/timezone paths are
// tokenised and overridden at build time; everything else (service_limits, etc.) is used as-is.
private val CONFIG_TEMPLATE = """
{
  ...
  "loki": {
    "actions": [
      ...
      "expansion","centroid","status","tile"
    ],
    ...
    "logging": {
      "type": "std_out","color": true,
      "file_name": "path_to_some_file.log","long_request": 100.0
    },
  ...
  "thor": {
    "logging": {
      "type": "std_out","color": true,
      "file_name": "path_to_some_file.log","long_request": 110.0
    },
    ...
    "costmatrix": {
      ...
      "min_iterations": 100,
      "hierarchy_limits": {
        "max_up_transitions": {"1":400,"2":100},
        "expand_within_distance": {"0":100000000.0,"1":20000,"2":5000}
      }
    },
    "bidirectional_astar": {
    ...
    "unidirectional_astar": {
      "hierarchy_limits": {
        "max_up_transitions": {"1":400,"2":100},
        "expand_within_distance": {"0":100000000.0,"1":100000,"2":5000}
      }
    }
  },
  "odin": {
  ...
  "meili": {
    "mode": "auto",
    "customizable": [
      "mode","search_radius","turn_penalty_factor","gps_accuracy",
      "interpolation_distance","sigma_z","beta",
      "max_route_distance_factor","max_route_time_factor"
    ],
    "verbose": false,
    "default": {
      "sigma_z": 4.07,"gps_accuracy": 5.0,"beta": 3,
      "max_route_distance_factor": 5,"max_route_time_factor": 5,
      "max_search_radius": 100,"breakage_distance": 2000,
      "interpolation_distance": 10,"search_radius": 50,
      "geometry": false,"route": true,"turn_penalty_factor": 0
    },
  ...
  "httpd": {
    "service": {
      "listen": "tcp://*:8002","loopback": "ipc:///tmp/loopback",
      "interrupt": "ipc:///tmp/interrupt","drain_seconds": 28,
      "shutdown_seconds": 1,"timeout_seconds": -1
    }
  },
  "service_limits": {
    ...
    "pedestrian": {"max_distance":250000.0,"max_locations":50,"max_matrix_distance":200000.0,"max_matrix_location_pairs":2500,"min_transit_walking_distance":1,"max_transit_walking_distance":10000},
    ...
    "trace": {"max_distance":200000.0,"max_gps_accuracy":100.0,"max_search_radius":100.0,"max_shape":16000,"max_alternates":3,"max_alternates_shape":100},
    ...
    "hierarchy_limits": {
      ...
      "costmatrix": {
        "max_allowed_up_transitions": {"1":400,"2":100},
        "max_expand_within_distance": {"0":100000000.0,"1":100000,"2":5000}
      },
      "unidirectional_astar": {
        "max_allowed_up_transitions": {"1":400,"2":100},
        "max_expand_within_distance": {"0":100000000.0,"1":100000,"2":5000}
      ...
      "bidirectional_astar": {
        "max_allowed_up_transitions": {"1":400,"2":100},
        "max_expand_within_distance": {"0":100000000.0,"1":20000,"2":5000}
      }
    },
